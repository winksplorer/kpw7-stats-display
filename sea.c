...
extern const char _binary_kpw7_stats_display_tar_xz_start[], _binary_kpw7_stats_display_tar_xz_end[];

int extract_mem(const void *data, size_t data_size, const char *output_dir) {
    // init libarchive + some variables
    struct archive* archive = archive_read_new(), * disk = archive_write_disk_new(); // archive = kpw7-stats-display.tar.gz, disk = /tmp/kpw7-stats-display
...
int main(int argc, char* argv[]) {
    // printing basic info
    printf("kpw7-stats-display SEA (%s %s).\nkpw7-stats-display and its SEA are licensed under the GNU GPL v2. copyright (c) 2025 winksplorer et al.\narchive size = %.2f MB\n",
        __DATE__, __TIME__, (double)(_binary_kpw7_stats_display_tar_xz_end - _binary_kpw7_stats_display_tar_xz_start)/1000000);

    // extract the embedded kpw7-stats-display.tar.xz into /tmp/kpw7-stats-display
    if (extract_mem(_binary_kpw7_stats_display_tar_xz_start, _binary_kpw7_stats_display_tar_xz_end - _binary_kpw7_stats_display_tar_xz_start, "/tmp/kpw7-stats-display") == -1)
        return fprintf(stderr, "archive extract failed\n"), 1;

    // pass through arguments
    argv[0] = "/tmp/kpw7-stats-display/kpw7-stats-display-server";
    execv("/tmp/kpw7-stats-display/kpw7-stats-display-server", argv);
